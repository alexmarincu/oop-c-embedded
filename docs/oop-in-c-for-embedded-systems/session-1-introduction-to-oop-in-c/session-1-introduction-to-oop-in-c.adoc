...
----
class Timer {
    + startTimeInMs: uint32_t
    + durationInMs: uint32_t
    + isRunning: bool
    + init(): Timer
    + start(durationInMs: uint32_t): void
    + stop(): void
    + isExpired(): bool
}
interface TimeSource {
    + getCurrentTimeInMs(): uint32_t
}
Timer .r.> TimeSource
----
...
void Timer_start(Timer * const self, uint32_t const durationInMs) {
    self->startTimeInMs = TimeSource_getCurrentTimeInMs();
...
bool Timer_isExpired(Timer const * const self) {
    return (
        (self->isRunning == true) &&
        ((TimeSource_getCurrentTimeInMs() - self->startTimeInMs) >= self->durationInMs)
    );
}
----
